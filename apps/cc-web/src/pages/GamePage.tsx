...
function getOverlayCopy(state: string) {
  if (state === 'crew-processing' || state === 'evaluating') {
    return { title: 'analyzing meaning', subtitle: 'transcribing both voices and comparing meaning' };
  }
  return null;
}
    ...
    <main className="game-screen">
      <div className="game-header brand-header">
        <div className="chunks-brand-block">
          ...
          <div>
            ...
            <h1 className="game-title">Tròn THC</h1>
          </div>
        </div>
      ...
      <section className="playfield-shell">
        <RolePanel
          ...
          processing={false}
          helperText={round.state === 'captain-ready' ? 'Speak in Vietnamese' : 'Captain turn'}
        ...
        <RolePanel
          ...
          helperText={round.state === 'crew-waiting' ? 'Reply in English before time runs out' : 'Crew turn'}
